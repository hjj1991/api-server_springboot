...
public class CategoryController {
    ...
    @GetMapping("/category")
    public ApiResponse categoryList(@AuthenticationPrincipal TokenDto user) {
        try {
            return ApiUtils.success(categoryService.findCategory(user.getUserNo()));
        } catch (Exception e) {
            log.error("categoryList Error userNo: {},  exception: {}", user.getUserNo(), e);
            return ApiUtils.error(ApiError.ErrCode.ERR_CODE9999.getMsg(), ApiError.ErrCode.ERR_CODE9999);
        }
    }
    ...
    public ApiResponse categoryDelete(@AuthenticationPrincipal TokenDto user,@ApiParam(value = "categoryNo", required = true) @PathVariable("categoryNo") Long categoryNo) {
        try {
            categoryService.deleteCategory(categoryNo, user.getUserNo());
